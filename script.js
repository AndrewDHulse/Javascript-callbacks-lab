...
//		3. Code the forEach method such that it iterates over each 
//         element in the array arg (use a for loop).
//		4. For each iteration, invoke the callback arg, passing to it, 
//         the element and the index of the element.
...
function step1(cb) {
  ...
  Write the code that invokes the three functions such that the 
  ...
  Hints: 
  - Call `step1` first.
  - You cannot call `step2` until after `step1` has "finished", 
  similarly, you cannot call `step3` until `step2` has "finished".
  - You must console.log the last line of the output, `FINISHED`, 
  after `step3` has "finished".
  */
